...
export function useOthello(initialSize = 8) {
    ...
    function cpuMove() {
        const mvs = boardRef.value.flatMap((row, y) => row.map((_, x) => ({ x, y })).filter(p => flips(p.x, p.y, turnRef.value).length > 0));
        if (!mvs.length) {
            pass();
            return;
        }
    ...
    const validMoves = computed(() => boardRef.value.flatMap((row, y) => row.map((_, x) => ({ x, y }))).filter(p => flips(p.x, p.y, turnRef.value).length > 0));
    const isGameOver = computed(() => passCount.value >= 2 || boardRef.value.flat().every(c => c !== null));
    const score = computed(() => {
        let b = 0, w = 0;
        boardRef.value.forEach(r => r.forEach(c => { if (c === 'black')
            b++;
        else if (c === 'white')
            w++; }));
