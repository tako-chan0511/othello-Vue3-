...
function cpuMoveOnce() {
    // 白手番の合法手を取得
    const moves = board.value.flatMap((row, y) => row.map((_, x) => ({ x, y }))).filter(p => flips(p.x, p.y, 'white').length > 0);
    if (!moves.length) {
        // 合法手がなければパス
        pass();
        return false;
...
function cpuMove() {
    ...
    const step = () => {
        if (turn.value !== 'white' || gameOver.value)
            return;
        if (cpuMoveOnce()) {
            // 一手打ったら次も白番か確認
            setTimeout(step, 300);
...
const score = computed(() => {
    let b = 0, w = 0;
    board.value.forEach(r => r.forEach(c => { if (c === 'black')
        b++;
    else if (c === 'white')
        w++; }));
